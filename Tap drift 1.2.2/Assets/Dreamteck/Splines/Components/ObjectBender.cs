...
namespace Dreamteck.Splines
{
    ...
    public class ObjectBender : SplineUser
    {
        ...
        public NormalMode upMode
        {
            ...
            set
            {
                if (computer != null && value != _normalMode)
                {
                    _normalMode = value;
                    Rebuild(false);
        ...
        public Vector3 customNormal
        {
            ...
            set
            {
                if (computer != null && value != _customNormal)
                {
                    _customNormal = value;
                    Rebuild(false);
                }
        ...
        public ForwardMode forwardMode
        {
            ...
            set
            {
                if (computer != null && value != _forwardMode)
                {
                    _forwardMode = value;
                    Rebuild(false);
        ...
        public Vector3 customForward
        {
            ...
            set
            {
                if (computer != null && value != _customForward)
                {
        ...
        public void UpdateReferences()
        {
            ...
            if (_bend)
            {
                ...
                for (int i = 0; i < bendProperties.Length; i++)
                {
                    bendProperties[i].Apply(i > 0 || trs != rootUser.computer.transform);
        ...
        private void GetBendResult(Vector3 percentage)
        {
            switch (axis)
            {
                case Axis.X: Evaluate(bendResult, UnclipPercent(percentage.x)); break;
                case Axis.Y: Evaluate(bendResult, UnclipPercent(percentage.y)); break;
                case Axis.Z: Evaluate(bendResult, UnclipPercent(percentage.z)); break;
            }
            switch (_normalMode)
            {
                case NormalMode.Auto: bendResult.normal = Vector3.Cross(bendResult.direction, bendResult.right); break;
                case NormalMode.Custom: bendResult.normal = _customNormal; break;
            }
            if (_forwardMode == ForwardMode.Custom) bendResult.direction = customForward;
            ...
            switch (axis)
            {
                ...
                case Axis.X:
                    ...
                    bendResult.position += right * Mathf.Lerp(bounds.max.z, bounds.min.z, percentage.z) * bendResult.size;
                    bendResult.position += bendResult.normal * Mathf.Lerp(bounds.min.y, bounds.max.y, percentage.y) * bendResult.size;
                    break;
                case Axis.Y:
                    axisRotation = Quaternion.Euler(90f, 0f, 0f);
                    bendResult.position += right * Mathf.Lerp(bounds.min.x, bounds.max.x, percentage.x) * bendResult.size;
                    bendResult.position += bendResult.normal * Mathf.Lerp(bounds.min.z, bounds.max.z, percentage.z) * bendResult.size;
                    break;
            }

            bendRotation = bendResult.rotation * axisRotation;
            normalMatrix = Matrix4x4.TRS(bendResult.position, bendRotation, Vector3.one * bendResult.size).inverse.transpose;
        }
        ...
        public void BendObject(BendProperty p)
        {
            ...
            if (p.originalSpline != null)
            {
                for (int n = 0; n < p.splinePointPercents.Length; n++)
                {
                    ...
                    switch (axis)
                    {
                        case Axis.X: point.normal = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * point.normal; break;
                        case Axis.Y: point.normal = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * point.normal; break;
        ...
        void BendMesh(Vector3[] vertexPercents, Vector3[] originalNormals, TS_Mesh mesh, Matrix4x4 worldToLocalMatrix)
        {
            ...
            for (int i = 0; i < mesh.vertexCount; i++)
            {
                ...
                GetBendResult(percent);
                mesh.vertices[i] = worldToLocalMatrix.MultiplyPoint3x4(bendResult.position);
