...
namespace Dreamteck.Splines
{
    ...
    public class ObjectBenderEditor : SplineUserEditor
    {
        ...
        protected override void BodyGUI()
        {
            ...
            if (bender.bendProperties.Length - 1 != childCount && !Application.isPlaying) bender.UpdateReferences();
            ...
            if(bender.upMode == ObjectBender.NormalMode.Custom) bender.customNormal = EditorGUILayout.Vector3Field("Custom Up", bender.customNormal);
            bender.forwardMode = (ObjectBender.ForwardMode)EditorGUILayout.EnumPopup("Forward Vector", bender.forwardMode);
            if (bender.forwardMode == ObjectBender.ForwardMode.Custom) bender.customForward = EditorGUILayout.Vector3Field("Custom Forward", bender.customForward);
            if (!bender.bend)
            {
        ...
        protected override void OnDestroy()
        {
            ...
            if (Application.isEditor && !Application.isPlaying)
            {
                ...
                {
                    if(!generatedUvs)  user.Rebuild(true);
                }
            }
